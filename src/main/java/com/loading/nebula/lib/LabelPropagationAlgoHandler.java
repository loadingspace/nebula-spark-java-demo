...
import com.loading.nebula.config.algo.LPAConfigProxy;
 ...
 * @date 2021/5/17
 */
public class LabelPropagationAlgoHandler extends AbstractGraphAlgoHandler {

  public static final String ALGO_NAME = "LabelPropagation";

  private LPAConfigProxy algoConfig;

  public LabelPropagationAlgoHandler(Configs configs) throws ConfigsIllegalException {
    super(configs);
    this.algoConfig = new LPAConfigProxy(configs.getAlgoConfig().getConfig());
  }
  ...
  protected Dataset<Row> afterExecuteAlgo(Dataset<Row> algoResult) {
    ...
    return algoResult.join(vData1,
        algoResult.col(Constants.ALGO_ID_COL).equalTo(vData1.col(Constants.V_LONG_ID)),
        "left")
        .join(vData2,
            algoResult.col(Constants.LPA_RESULT_COL).equalTo(vData2.col(Constants.V_LONG_ID)),
            "left");
  }
